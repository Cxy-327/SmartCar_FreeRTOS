...
//        reg_int_cb(int_param);
    ...
    /* Already disabled by setup_compass. */
		printf ("here \n");
    if (mpu_set_bypass(0))		//这里又卡住	//又是改hal_delay
        return -1;
		printf ("OKOKOKOKOOk\n");
#endif
		printf ("go to mpu_set_sensor \n");

    mpu_set_sensors(0);
		printf ("mpu_set_sensors  over\n");
    return 0;
}
...
int mpu_reset_fifo(void)
{
    ...
    if (st.chip_cfg.dmp_on) {printf ("into here   \n");

        data = BIT_FIFO_RST | BIT_DMP_RST;
        if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &data))
            return -1;
        //HAL_Delay(50);
				mpuiic_Delayus(5000);//5000
				printf ("delay over");
...
int mpu_configure_fifo(unsigned char sensors)
{
    ...
    if (st.chip_cfg.dmp_on)
        ...
        if (sensors || st.chip_cfg.lp_accel_mode)
				{
					//printf("set_init_enable(1)\n");
					set_int_enable(1);
					//printf("set_init_enable(1)   over\n");
				}
            
        else
				{
					//printf("set_init_enable(0)\n");
				    set_int_enable(0);
					//printf("set_init_enable(0)   over\n");
				}
        
				//printf ("sensors=%d\n",sensors );
        if (sensors) {//printf ("go to mpu_reset_fifo\n");
            if (mpu_reset_fifo()) {
							//printf ("mpu_reset_fifo  over  !=0\n");
                st.chip_cfg.fifo_enable = prev;
                return -1;
            }//printf ("mpu_reset_fifo over  ==0\n");
        }
    }
  //printf ("mpu_configure_fifo  over  return result==%d",result );
    return result;
}
...
int mpu_set_sensors(unsigned char sensors)
{
    ...
    if (sensors && (sensors != INV_XYZ_ACCEL))
		...
		//printf("go to  mpu_set_int_latched\n");
        mpu_set_int_latched(0);
		//printf ("mpu_set_int_latched   over\n");
#ifdef AK89xx_SECONDARY
#ifdef AK89xx_BYPASS
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    if (i2c_write(st.hw->addr, st.reg->fifo_en, 1, data))
        return -1;
    //HAL_Delay();
		mpuiic_Delayus(test.wait_ms*100);
    data[0] = 0;
    if (i2c_write(st.hw->addr, st.reg->fifo_en, 1, data))
        return -1;
...
{
	uint8_t res=0;
	//printf ("go to MPU IIC Init\n");
	MPU_IIC_Init(); 	//初始化IIC总线
	//printf ("MPU IIC init over\n");
	//printf ("go to mpu_init\n");	
	if(mpu_init()==0)	//初始化MPU6050   /////md就是这里卡住了
	{	 
		//printf("mpu_init over   ==0\n");
		
		res=mpu_set_sensors(INV_XYZ_GYRO|INV_XYZ_ACCEL);//设置所需要的传感器
		//printf ("mpu_set_sensors  over\n");
		//printf ("res=%d",res);
		if(res)return 1; 
		//printf ("into mpu_configure_fifo\n");
		res=mpu_configure_fifo(INV_XYZ_GYRO|INV_XYZ_ACCEL);//设置FIFO		//到这里了  return 0
		//printf (" mpu_configure_fifo   over  res=%d \n",res);
